...
const StepThree = ({ data, onNext, onBack }) => {
  ...
  // useEffect to initialize state based on data prop
  useEffect(() => {
    if (data) {
      setJob(data.job || '');
  ...
  const handleNext = () => {
    ...
    if (isStepThreeValid || noExperience) {
      // Validate github and linkedin as URLs only if provided
      const isGithubValid = !github || isValidUrl(github);
      const isLinkedinValid = !linkedin || isValidUrl(linkedin);
  
      if (!isGithubValid) {
        // Handle invalid github URL
        console.error('Invalid GitHub URL');
        return;
      }
  
      if (!isLinkedinValid) {
        // Handle invalid linkedin URL
